...
import { executeSearch, searchAlgorandInfo } from "../_core/mcpSearch";
...
const agentTools: Tool[] = [
  ...
  },
  {
    ...
    description: "Look up Algorand blockchain information",
    parameters: {
      type: "object",
      properties: {
        query: {
          type: "string",
          description: "The blockchain query (e.g., account info, transaction, asset ID)",
        },
      },
      required: ["query"],
    },
...
export const agentRouter = router({
  ...
  chat: publicProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      const systemPrompt = `You are an Algorand blockchain AI agent. You have access to tools:
1. search — search curated Algorand / web documentation (results may be demo data).
2. blockchain_lookup — Algorand-focused lookup (demo data until live MCP is connected).
...
${input.walletPublicKey ? `The user's Algorand wallet public key is: ${input.walletPublicKey}` : "No wallet is connected; explain when signing or account-specific actions need connection."}

${repBlock}

Payments: The client may use per-request (x402-style) quotes, prepaid credits, or session/streaming settlement. x402 on Algorand expects atomic transaction groups. Mention when an action looks unpaid, pending authorization, or settled only if the user’s message or context implies it; do not invent transaction IDs.
...
After tools run, answer concisely and reference what you learned.`;
  ...
  search: publicProcedure
    .input(
      z.object({
        query: z.string(),
        searchType: z.enum(["general", "algorand", "blockchain"]).default("general"),
      })
    )
    .mutation(async ({ input }) => {
      try {
        if (input.searchType === "algorand") {
          return await searchAlgorandInfo(input.query);
        }
  ...
  blockchainLookup: publicProcedure
    ...
    .mutation(async ({ input }) => {
      try {
        const response = await searchAlgorandInfo(input.query);
        return {
          query: input.query,
...
async function runToolCall(
  ...
  try {
    ...
    if (fn === "blockchain_lookup") {
      const q = String(args.query ?? "");
      const r = await searchAlgorandInfo(q);
      recordedTools.push({
        ...base,
