...
export async function settleChallenge(params: ApproveParams): Promise<ApproveResult> {
  ...
  // Handle Algorand x402 settlement via facilitator
  let txRef = `sim_${Date.now()}_${nanoid(10)}`;
  if (params.signedTxn) {
    try {
      const result = await facilitator.settle(params.signedTxn);
      txRef = result.txId || txRef;
    } catch (error: any) {
      store.releaseReplayKey(params.replayKey);
      throw new PaymentLayerError("settlement_failed", `Algorand settlement failed: ${error.message}`);
    }
  } else if (row.quote.mode === "prepaid_credits") {
    try {
