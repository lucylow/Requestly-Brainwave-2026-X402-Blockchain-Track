/**
 * Shared payment vocabulary for Algorand x402 / MPP-style flows.
...
export type AlgorandPaymentNetwork = "mainnet" | "testnet" | string;

export type PaymentAsset = {
  id: string; // Asset ID for Algorand
  code: string;
};
...
export type PaymentQuote = {
  ...
  /** Wallet must support Algorand signing for real x402 */
...
export type PaymentAuthorization = {
  challengeId: string;
  payer: string;
  /** Signed transaction for Algorand x402 */
  signedTxn?: string;
  authorizedAt: string;
  replayKey: string;
};
