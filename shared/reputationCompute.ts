...
import type { AlgorandOperationRecord, AlgorandTxRecord } from "./appConnectionModel";
...
export function onChainSuccessRateFromTx(transactions: AlgorandTxRecord[]): number | null {
  if (!transactions.length) return null;
  const ok = transactions.filter((t) => t.successful).length;
  return ok / transactions.length;
}

export function accountActivityTrendFromOps(ops: AlgorandOperationRecord[]): "busy" | "quiet" | "unknown" {
