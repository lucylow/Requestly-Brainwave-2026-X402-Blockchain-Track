...
export function agentStepNarrative(
  ...
  if (task?.stage === "wallet_needed" && !walletConnected) {
    return "I’m looking up your account context — connect Pera Wallet for live Algorand data.";
  }
  ...
  if (task?.stage === "settling" || activity === "settling") {
    return "Settling the demo payment trace on Algorand…";
  }
  if (task?.stage === "searching" || activity === "waiting_search") {
    return "I’m checking the latest Algorand information from search sources.";
  }
  if (task?.stage === "fetching_account" || activity === "looking_up_blockchain") {
    return `I’m running a Algorand blockchain lookup${net} — structured snippets, not raw Indexer JSON.`;
  }
  ...
  if (activity === "tool_selection" || activity === "calling_tool") {
    return "I’m choosing and running an agent tool (search or Algorand lookup).";
  }
  ...
  if (activity === "idle") {
    return "Ready when you are — ask about Algorand, wallets, or try a demo x402 prompt.";
  }
