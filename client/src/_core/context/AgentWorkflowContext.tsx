...
export function AgentWorkflowProvider({ children }: { children: ReactNode }) {
  ...
  const dispatchAgent = useCallback(
    (event: AgentStateEvent) => {
      ...
      if (event.type === "request_start") {
        ...
        pushTimeline(event, "Awaiting wallet authorization", "Algorand wallet auth entry (simulated)");
      } else if (event.type === "settling") {
        pushTimeline(event, "Settling on Algorand", "Demo settlement trace");
  ...
  const beginTurn = useCallback(
    (userQuery: string, meta?: BeginTurnMeta) => {
      ...
      if (meta?.requiresPayment) {
        ...
        stage = "wallet_needed";
        currentAction = "Wallet context recommended — connect wallet for live Algorand data";
      } else {
        stage = "queued";
  ...
  const recordToolCalls = useCallback(
    (tools: AgentToolCallWire[]) => {
      ...
      if (hasBc) {
        emitTimeline("tool_called", "Algorand: account / balance context", "Aligns chat tool with Indexer-style facts");
        emitTimeline("tool_called", "Algorand: transactions (if applicable)", "Structured lookup path");
        emitTimeline("tool_called", "Algorand: network sanity", "Testnet vs mainnet wording");
      }
