...
type ReputationContextValue = {
  ...
  setIndexerSuccessRate: (rate: number | null) => void;
...
export function ReputationProvider({ children }: { children: ReactNode }) {
  const { isConnected, publicKey, refreshError, account } = useAlgorandWallet();
  const [root, setRoot] = useState<PersistedRoot>({ byAccount: {} });
  const [hydrated, setHydrated] = useState(false);
  const [horizonSuccessRate, setIndexerSuccessRate] = useState<number | null>(null);
