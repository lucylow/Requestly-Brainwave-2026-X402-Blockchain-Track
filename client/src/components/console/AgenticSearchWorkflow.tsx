...
import { useAlgorandAgent } from "@/contexts/AlgorandAgentContext";
...
export function AgenticSearchWorkflow({ policy, serviceId }: AgenticSearchWorkflowProps) {
  const { pushActivity } = useAlgorandAgent();
  const { appendPaymentReceipt, emitTimeline } = useAgentWorkflow();
  const { network } = useAlgorandWallet();
  ...
  const networkLabel =
    mode === "mock" ? "Algorand testnet (mock settlement)" : network === "mainnet" ? "Algorand mainnet" : "Algorand testnet";
  ...
  return (
    <Card className="surface-card border-[var(--border-strong)] p-4 space-y-4 scroll-mt-28" id="console-search">
      ...
      <InlineAlert variant="info" title="Transparent pricing">
        Connect a wallet to authorize live Algorand actions elsewhere in the app. This panel always shows estimate → approval →
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              placeholder="Try “Algorand consensus”, “Soroban”, or “USDC on Algorand”"
