      ...
      <p className="text-xs text-slate-300 leading-relaxed">
        {synthesisIntro ??
          (isDemo
            ...
            : "I searched the web for the latest Algorand information. These sources are the strongest matches.")}
      </p>
...
function interpretBlockchainCopy(data: SearchResponseWire): string[] {
  ...
  if (data.results.length === 0) {
    ...
    lines.push("Lookup returned curated snippets you can cross-check on Indexer or Algorand.expert.");
  }
  if (/\b(testnet|mainnet)\b/i.test(q)) {
    lines.push(`The request mentions network context — confirm ${/\bmainnet\b/i.test(q) ? "mainnet" : "testnet"} in Pera Wallet.`);
  }
  if (/\b(balance|xlm)\b/i.test(q)) {
    lines.push("For real balances, connect Pera Wallet and open the dashboard tab.");
  }
  ...
  return (
    <div className="space-y-3 rounded-lg border border-purple-500/40 bg-purple-950/20 p-3">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <Badge variant="outline" className="text-[9px] border-slate-500/45 text-slate-300">
          Not a wallet balance — cross-check Indexer
        </Badge>
        {usedByAgent && (
          <Badge variant="outline" className="text-[9px] border-emerald-500/35 text-emerald-200">
            Used by agent
      ...
      <p className="text-xs text-slate-300 leading-relaxed">
        {synthesisIntro ??
          (isDemo
            ? "Structured Algorand-oriented snippets (demo). Cross-check on a block explorer before acting."
            : "Live-oriented Algorand lookup context. Plain-language readout below; verify critical amounts on Indexer.")}
      </p>
      ...
      <p className="text-[10px] text-slate-600">
        ...
        <span className="font-mono text-slate-500">searchAlgorandInfo</span> (same shape as web search cards; not raw
        Indexer JSON).
      </p>
