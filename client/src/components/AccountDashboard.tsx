...
import type { AlgorandTxRecord, AlgorandOperationRecord } from "@shared/appConnectionModel";
...
import { interpretAlgorandAccountPanel } from "@shared/blockchainInterpret";
...
export function AccountDashboard({ onConnectRequest }: AccountDashboardProps) {
  ...
  const { summary, setIndexerSuccessRate, hydrated } = useReputation();
  const w = useAlgorandWallet();
  ...
  useEffect(() => {
    if (!w.transactions.length) {
      setIndexerSuccessRate(null);
      return;
    }
    setIndexerSuccessRate(onChainSuccessRateFromTx(w.transactions));
  }, [w.transactions, setIndexerSuccessRate]);

  const accountErr = w.refreshError
    ? { title: "Refresh issue", userMessage: w.refreshError }
    : null;
  ...
  const interpretationLines = interpretAlgorandAccountPanel({
    account: w.account,
    network,
  ...
  const traceItems = [
    {
      ...
      detail: publicKey ? truncateAddress(publicKey, 8, 6) : undefined,
    ...
    },
    {
      ...
      detail: latestTx
        ? `${truncateAddress(latestTx.hash, 8, 6)} · ${latestTx.successful ? "Success" : "Failed"}`
        : w.transactionsError
          ? "Could not load"
    ...
    {
      id: "op",
      label: "Latest transaction detail in view",
      detail: latestOp
        ? `${latestOp.type} · ${truncateAddress(latestOp.transaction_hash, 8, 6)}`
        : w.operationsError
          ? "Could not load"
  ...
  if (!publicKey) {
    return (
      <EmptyState
        title="Connect Wallet to view Algorand account data"
        body="You’ll see live ALGO balance, recent transactions, and operations from Algorand for Testnet or Mainnet. Chat still works in demo mode without a wallet."
        action={
          onConnectRequest ? (
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Button
            ...
            aria-label="Refresh Algorand account and transactions"
          >
      ...
      >
          ...
          Links wallet refresh, recent transactions, and where to continue in chat — not a full explorer.
        </p>
      ...
      <SectionCard title="Account identity" icon={<User className="w-4 h-4 text-cyan-400" aria-hidden />}>
        {w.isAccountSyncing && !w.account ? (
          <div className="space-y-2" aria-busy aria-label="Loading account identity">
            <InlineLoading label="Fetching account identity from Algorand…" className="py-1" />
          ...
          <div className="space-y-2 text-sm">
            <div className="flex flex-wrap items-center justify-between gap-2">
              ...
              <div className="flex items-center gap-1">
                <code className="text-xs font-mono text-cyan-300">{truncateAddress(publicKey, 10, 8)}</code>
                <Button
                  type="button"
                  size="icon"
                  variant="ghost"
            ...
            <p className="text-[11px] text-slate-500">
              Live account data from Algorand on {networkLabel}.
            </p>
          </div>
        ) : (
          <Alert className="border-amber-500/30 bg-amber-950/15">
            ...
            <AlertDescription className="text-sm text-amber-100/85">
              {accountErr?.userMessage ??
                "This address may be unfunded on this network, or the Algorand node is temporarily unavailable."}
            </AlertDescription>
            <Button
              type="button"
              size="sm"
      ...
      >
        {w.isAccountSyncing && !w.account ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <MetricCard
              label="ALGO"
              value={<span className="font-mono text-cyan-300">{w.balance} ALGO</span>}
              hint="Spendable ALGO balance on this network."
            />
            <MetricCard
              label="Address"
              value={
                <span className="font-mono text-cyan-300/90 text-xs truncate" title={w.account.address}>
                  {truncateAddress(w.account.address, 12, 8)}
                </span>
              }
            />
          </div>
      ...
      >
        {w.isFetchingNetwork && !w.networkInfo ? (
          <InlineLoading label="Fetching latest ledger and fee data from Indexer…" />
        ) : w.networkInfoError ? (
          <ErrorState
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
            <MetricCard
              ...
              hint="Tip of the ledger this Indexer node reports."
            />
            <MetricCard label="Min balance (microAlgos)" value={w.networkInfo.baseReserve} />
        ...
        <div>
          ...
          {w.isFetchingTransactions && w.transactions.length === 0 ? (
            ...
            <p className="text-sm text-[var(--muted-text)] py-2">
              No recent transactions in this Indexer window — this account may be new or quiet on {networkLabel}.
            </p>
          ) : (
            <ScrollArea className="h-48 rounded-md border border-purple-500/15">
              <ul className="space-y-2 p-2 pr-4">
                {w.transactions.map((tx: AlgorandTxRecord) => (
                  <li
                    key={tx.id}
          ...
          {w.isFetchingOperations && w.operations.length === 0 ? (
            ...
            <ScrollArea className="h-36 rounded-md border border-cyan-500/15">
              <ul className="space-y-1.5 p-2 pr-4">
                {w.operations.map((op: AlgorandOperationRecord) => (
                  <li
                    key={op.id}
      ...
      >
        ...
        <p className="text-xs text-slate-500">
          Payment middleware ledger (this server). Distinct from Indexer activity above until live x402 is wired.
        </p>
        <PaymentHistory />
...
function CardFootnote() {
  return (
    <div className="rounded-lg border border-cyan-500/20 bg-slate-950/60 p-3 flex items-center gap-2 text-xs text-slate-500">
      ...
      <span>
        Dashboard reads the same network mode as the wallet card. If one Indexer call fails, others can still show —
