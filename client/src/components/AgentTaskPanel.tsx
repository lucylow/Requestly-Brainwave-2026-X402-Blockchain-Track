...
export function AgentTaskPanel({ embedded }: { embedded?: boolean }) {
  ...
  const blockchainStageLines = [
    "Wait for wallet if the prompt needs your address or live balances.",
    "Select blockchain_lookup — server runs searchAlgorandInfo (structured snippets, often demo).",
    "Optional: cross-check the account dashboard for raw Indexer transactions.",
          ...
          <>
            ...
            {currentTask.needsWallet && (
              <p className="text-[11px] text-amber-200/90 flex items-center gap-1.5">
                ...
                Wallet context recommended for this prompt — answers may be generic without a wallet connection.
              </p>
            )}
            ...
            <p className="text-[10px] text-slate-600">
              Detailed Indexer rows appear in the account dashboard; the agent uses structured lookup output.
            </p>
              </div>
            )}
