...
import { buildAlgorandContextForLlm } from "@/_core/ai/algorandContextForLlm";
...
export function AgentChat({ walletPublicKey }: { walletPublicKey?: string }) {
  ...
  const stellarWallet = useAlgorandWallet();
  const { isConnected, networkLabel, algorandWalletDetected, network, authEntrySigningAvailable } = stellarWallet;
  const reputation = useReputation();

  const [paymentDemoFree, setPaymentDemoFree] = useState(() => {
    if (typeof window === "undefined") return false;
    return window.localStorage.getItem("algorand_payment_demo_free") === "1";
  });
  const [paymentModePref, setPaymentModePref] = useState<
    "auto" | "per_request" | "prepaid_credits" | "session_streaming" | "demo_free"
  >(() => {
    if (typeof window === "undefined") return "auto";
    const v = window.localStorage.getItem("algorand_payment_mode_pref");
  ...
  const runAssistantReply = useCallback(
    async (userText: string, conversationHistory: HistoryTurn[]) => {
      ...
      const algorandContext = buildAlgorandContextForLlm(stellarWallet);
      ...
      const response = await chatMutation.mutateAsync({
        ...
        stellarContext: algorandContext,
      });
      ...
      for (const tc of toolCalls) {
        if (tc.status === "completed" && tc.result && isSearchResponseWire(tc.result)) {
          ...
          toolCards.push({
            id: `t_${tc.id}`,
            role: "tool",
            content: isSearch ? "Agent search results" : "Agent blockchain / Algorand lookup results",
            timestamp: new Date(),
            toolCalls: [tc],
            demoLabel: isSearch
              ? searchMode === "mock"
    ...
    async (messageId: string, payment: PaymentPayload) => {
      ...
      dispatchAgent({ type: "payment_authorizing" });
        patchCurrentTask({
          ...
          currentAction: "Awaiting wallet authorization (demo)",
          progress01: 0.35,
        });
      ...
      try {
        ...
        patchCurrentTask({
          stage: "settling",
          currentAction: "Settling payment on Algorand (simulated)",
          progress01: 0.48,
        });
        ...
        const unlockedHint =
          "The request is now unlocked and I can continue with search / Algorand lookup tools.";
  ...
  const chatBlockchainSurface = deriveBlockchainSurfaceState({
    ...
    freighterDetected: stellarWallet.algorandWalletDetected,
  ...
  const paymentAuthBlocked =
    algorandWalletDetected === false
      ? "Install an Algorand wallet extension."
      : !isConnected
        ? "Connect wallet for a payer address on this network."
        : !authEntrySigningAvailable
          ? "This wallet does not expose a signing API."
          : undefined;
      ...
      <div className="border-b border-[var(--border)] bg-[var(--surface)]/90 px-4 py-3 space-y-2 shrink-0">
          ...
          <PaymentModeToggle
            ...
            onDemoFreeChange={(v) => {
              ...
              if (typeof window !== "undefined") {
                window.localStorage.setItem("algorand_payment_demo_free", v ? "1" : "0");
              }
            }}
            onPreferredModeChange={(m) => {
              setPaymentModePref(m);
              if (typeof window !== "undefined") {
                window.localStorage.setItem("algorand_payment_mode_pref", m);
              }
            }}
          />
          {!reputation.hydrated ? (
        ...
        >
          ...
          <p className="text-xs text-slate-300 leading-relaxed">
            {isLoading
              ? predictedRouteRef.current === "blockchain_lookup" || predictedRouteRef.current === "mixed"
                ? "Blockchain lookup in progress — waiting for the model and Algorand tool… "
        ...
        <div className="space-y-4 pr-2" role="log" aria-label="Agent conversation" aria-relevant="additions">
          {messages.length === 0 && (
            <div className="text-center text-slate-500 py-10 space-y-2" role="status">
              <p className="text-sm text-slate-400">Ask about Algorand, or try “x402 paid search for Soroban docs”.</p>
              <p className="text-xs text-slate-500">
                {!walletPublicKey && "Connect Pera Wallet so the agent can use your public key and realistic payment context."}
              </p>
            </div>
          )}
                  ...
                  <PaymentChallengeCard
                    ...
                    blockReason={
                      message.payment.authEntrySigningRequired
                        ? paymentAuthBlocked
                        : !isConnected
                          ? "Connect Pera Wallet so this settlement is attributed to your address."
                          : undefined
                    }
                ...
                isSearchResponseWire(message.toolCalls[0].result) ? (
                  message.toolCalls[0].type === "search" ? (
                    ...
                    <BlockchainAgentResultCard
                      ...
                      synthesisIntro="Algorand / Indexer-oriented lookup using the agent tool."
                      usedByAgent
                      priorSuccessUrlSet={reputation.priorSuccessfulSearchUrls}
                    />
                  )
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="text-slate-500">
                            ...
                            {tool.type === "blockchain_lookup" && tool.status === "completed" && (
                              <span className="block text-[10px] text-slate-600 mt-0.5">
                                Procedure: agent.blockchainLookup → searchAlgorandInfo (structured Algorand snippets;
                                demo until live MCP).
                              </span>
                            )}
      ...
      <div className="border-t border-purple-500/20 p-3 bg-slate-900/40 shrink-0">
        ...
        <div className="flex gap-2 items-end">
          <Textarea
            ...
            aria-label="Message to Algorand AI agent"
          />
          <Button
            type="button"
            onClick={() => void send()}
